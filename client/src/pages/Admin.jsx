...
const Admin = ({ user }) => {
  ...
  const [newArrivalFormData, setNewArrivalFormData] = useState(
  ...
  const normalizeProductData = (product) => ({
    ...
    customizable:
      product.customizable === true
        ...
        ? "no"
        : product.customizable || "no",
    newArrival:
      product.newArrival === true
        ? "yes"
        : product.newArrival === false
        ? "no"
        : product.newArrival || "no",
  });
  ...
  const handleAddNewArrival = async (formData) => {
    ...
    try {
      ...
      if (response.data.status !== "success")
        throw new Error(response.data.message || "Failed to add new arrival");
  ...
  const handleEditNewArrival = async (formData) => {
    try {
      ...
      if (response.data.status !== "success")
        throw new Error(
          response.data.message || "Failed to update new arrival"
        );
  ...
  const handleDeleteNewArrival = async (id) => {
    if (newArrivals.length <= 5) {
      setError(
        "Cannot delete new arrival: Minimum of 5 new arrivals required."
      );
      setSuccess(null);
      return;
    }
    ...
    try {
      ...
      if (response.data.status !== "success")
        throw new Error(
          response.data.message || "Failed to delete new arrival"
        );
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50">
      ...
      <NewArrivalModal
        ...
        onSubmit={
          isEditingNewArrival ? handleEditNewArrival : handleAddNewArrival
        }
